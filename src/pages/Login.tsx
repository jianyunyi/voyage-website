  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const result = mode === "login"
        ...
        : await registerUser(email, password, email.trim().split("@")[0] || email.trim());
      if (!result.success) {
        setError(result.message);
        return;
      }
      if (mode === "login") login(result.user);
      else register(result.user);
      navigate("/", { replace: true });
    } catch (err) {
      ...
      <div className="relative z-10 min-h-[100dvh] flex items-center justify-center px-4 py-16">
        <div className="w-full max-w-md">
          ...
          >
            ...
            >
              <div>
                <label htmlFor="email" className="block text-sm font-medium text-white/85 mb-1.5">邮箱</label>
