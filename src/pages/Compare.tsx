...
import { Plane, Car, Building, Check, Info, Search, MapPin, Calendar, Users, Loader2, Star, Hotel } from "lucide-react";
...
export function getResultSourceLabel(item: CompareItem): string {
  const hotel = item as HotelItem;
  return isLiveResult(item) ? "实时数据" : hotel.dataSource === "fallback" || hotel.source === "fallback" ? "估算/兜底数据" : "参考数据";
}
...
function HotelCard({ item, index, onBook, onSubscribe, subscribed, subscribing }: {
  ...
  const image = resolveHotelImage({ image: item.publisherImage || item.image, images: item.publisherImages?.length ? item.publisherImages : item.images });
  return <motion.article initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: index * 0.08 }} className={`surface surface-hover overflow-hidden ${index === 0 ? "border-orange-300 ring-1 ring-orange-100" : ""}`}>
    <div className="grid gap-0 lg:grid-cols-[minmax(280px,1.1fr)_minmax(260px,1fr)_180px]">
      <div className="hotel-image-overlay relative min-h-56 overflow-hidden bg-stone-200 lg:min-h-full"><img src={image} alt={title} loading="lazy" onError={handleHotelImageError} className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/65 via-black/10 to-transparent" aria-hidden="true" /><div className="absolute left-4 top-4 flex gap-2"><span className={`rounded-full px-3 py-1 text-xs font-bold ${isLiveResult(item) ? "bg-emerald-100 text-emerald-800" : "bg-stone-900/75 text-white"}`}>{getResultSourceLabel(item)}</span>{index === 0 && <span className="rounded-full bg-orange-500 px-3 py-1 text-xs font-bold text-white">全网最低</span>}</div><p className="absolute bottom-4 left-4 right-4 font-serif text-lg font-bold text-white drop-shadow">{title}</p></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!isLoading && !error && hasSearched && results.length === 0 && (
          <div className="surface flex flex-col items-center justify-center px-6 py-16 text-center">
            ...
            <h2 className="font-serif text-2xl font-bold">没有找到匹配结果</h2>
            <p className="mt-2 text-sm text-stone-500">调整目的地或日期后，再试一次搜索。</p>
          </div>
        )}
        {!isLoading && !error && !hasSearched && (
          <div className="surface flex flex-col items-center justify-center px-6 py-16 text-center"><Search className="mb-4 h-10 w-10 text-orange-500" /><h2 className="font-serif text-2xl font-bold">开始探索价格</h2><p className="mt-2 text-sm text-stone-500">填写条件并搜索，查看不同平台的实时与估算方案。</p></div>
        )}
